...
> 📄 Paper: *An Approximation of Word Mover Distance Using a Neural Network Model* :contentReference[oaicite:0]{index=0}
...
Instead of directly approximating WMD with heuristics (e.g., averages of bounds),  
...
This reframes the problem from absolute regression → structured prediction.

🏗 Model
Input: two sentences (word embedding sequences)
Encoder: Bi-directional GRU
Interaction: Attention mechanism
Output: scalar ratio r ∈ [0,1] (via sigmoid)
...
Efficient approximation using existing bounds
📊 Results (MSE)
...
Even weak lower bounds (RWMD) become competitive with learning
⚡ Efficiency
Method	Runtime (ms)
WMD (exact)	93.6
Proposed	2.4
